...
!function($) {
	...
	var Datetimepicker = function(element, options) {
		...
		this.component = this.element.is('.date') ? this.element.find(
				'.add-on .icon-th, .add-on .icon-time, .add-on .icon-calendar')
				.parent() : false;
		this.componentReset = this.element.is('.date') ? this.element.find(
				'.add-on .icon-remove').parent() : false;
	...
	Datetimepicker.prototype = {
		...
		fill : function() {
			...
			if (this.formatViewType == "time") {
				...
				var hoursDisplay = (hourConverted < 10 ? '0' : '')
						+ hourConverted;
				var minutesDisplay = (minutes < 10 ? '0' : '') + minutes;
				var meridianDisplay = dates[this.language].meridiem[hours < 12 ? 0
						: 1];
				...
				this.picker.find('.datetimepicker-hours thead th:eq(1)').text(
						dayMonth + ' ' + dates[this.language].months[month]
								+ ' ' + year);
				this.picker.find('.datetimepicker-minutes thead th:eq(1)')
						.text(
								dayMonth + ' '
			...
			for (var i = 0; i < 24; i++) {
				...
				if (this.showMeridian
					...
					txt = i + ':00';
					html.push('<span class="hour' + clsName + '">' + txt
							+ '</span>');
				}
			}
			...
			for (var i = 0; i < 60; i += this.minuteStep) {
				...
				if (this.showMeridian
					...
					html.push('<span class="minute' + clsName + '">' + txt
							+ ':' + (i < 10 ? '0' + i : i) + '</span>');
					if (i == 59) {
						html.push('</fieldset>');
					}
					...
					html.push('<span class="minute' + clsName + '">' + hours
							+ ':' + (i < 10 ? '0' + i : i) + '</span>');
				}
			}
			...
			var yearCont = this.picker.find('.datetimepicker-years').find(
		...
		showMode : function(dir) {
			if (dir) {
				var newViewMode = Math.max(0, Math.min(
						DPGlobal.modes.length - 1, this.viewMode + dir));
	...
	var DPGlobal = {
		...
		parseDate : function(date, format, language, type) {
			...
			var parts = date && date.match(this.nonpunctuation) || [], date = new Date(
					0, 0, 0, 0, 0, 0, 0), parsed = {}, setters_order = [ 'hh',
					'h', 'ii', 'i', 'ss', 's', 'yyyy', 'yy', 'M', 'MM', 'm',
					'mm', 'D', 'DD', 'd', 'dd', 'H', 'HH', 'p', 'P' ], setters_map = {
				hh : function(d, v) {
...
function initDatePickerForDaySlash() {
...
function initDatePickerForHourSlash() {
	$(".form_datetime_hour").datetimepicker({
		language : 'zh-CN',
		format : "yyyy/mm/dd hh:ii",
		autoclose : true,
