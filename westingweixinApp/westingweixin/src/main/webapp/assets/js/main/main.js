	...
	for (var i = 0; i < arryData.length; i++) {
		if (arryData[i].type == 0) {
			t = t + "<div class='menu_head' id='menu_head" + arryData[i].id
					+ "' onclick='javascript:changeMenuParent("
					+ arryData[i].id + ")'>";
			t = t + "	<table><tr>";
			t = t
...
function genMenuChild(list, id) {
	...
	for (var i = 0; i < list.length; i++) {
		if (list[i].type != 0 && list[i].menuId == id) {
			if (list[i].active == 0) {
				t = t + "    <div class='menu_body_a' id='menu_body_a"
						+ list[i].id + "' onclick='javascript:changeMenuChild("
						+ list[i].id + ")'>　　　" + list[i].text + "</div>";

			} else {
				t = t + "    <div class='menu_body_a_disable' id='menu_body_a"
						+ list[i].id + "' >　　　" + list[i].text + "</div>";
			}
		}
	}
...
function changeMenu(menuId) {
	...
	$(".page_content")
			.append(
					"<div class=\"page_left\"><div class=\"menuLeft\" id=\"menuLeft\"></div></div><div class=\"page_right\" style=\"font-size: 0;\"><iframe frameborder=\"0\" height=100% width=100%  style=\"margin-right: -3px;\"  id=\"iframeMain\"></iframe></div>");
	$("#menuLeft").empty();
	var k = 0;
	for (var i = 0; i < arryData.length; i++) {
		if (arryData[i].type == 0)
		...
		k++;
		$("#menuLeft").append(
				"<a href='javascript:;' onclick='turnToPage(\""
						+ arryData[i].src + "\"," + arryData[i].id
						+ ")' id='leftMenu" + arryData[i].id + "'>"
						+ "<div><img src='" + basePath + "/img/main/1.png'/>"
						+ arryData[i].text + "</div></a>");
		if (k == 1)
