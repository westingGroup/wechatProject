...
public class ProcessController {
	...
	public ModelAndView index() {
		...
		// 服务需求方新注册人员
		processCount.setDemanderRegisterCount(demanderService
				.getDemanderTotalByConditions(demanderModel));
		// 服务提供商新注册人员
		processCount.setProviderRegisterCount(providerService
				.getDemanderTotalByConditions(demanderModel));
		// 新需求
	...
	public @ResponseBody String listByPage(String currentPage, String pageSize,
			String linkname, String linkphone, String type, String status,
			HttpSession session) {
		...
		demanderSearchModal.setLinkname(StringUtils.isBlank(linkname) ? ""
				: linkname.trim());
		demanderSearchModal.setLinkphone(StringUtils.isBlank(linkphone) ? ""
				: linkphone.trim());
		demanderSearchModal
		...
		if (type.equals("demander")) {
			userResult = demanderService
					.listDemanderByKeyword(demanderSearchModal);
		} else if (type.equals("provider")) {
			userResult = providerService
					.listProviderByKeyword(demanderSearchModal);
		} else if (type.equals("inside")) {
			demanderSearchModal.setUsername(StringUtils.isBlank(linkname) ? ""
					: linkname.trim());
			demanderSearchModal.setPhone(StringUtils.isBlank(linkphone) ? ""
					: linkphone.trim());
			userResult = insideProviderService
	...
	public @ResponseBody String dealRegister(String type, String demanderIds,
			String remark, int dealType, HttpSession session) {
		String rtnStr = "操作失败";
	...
	public @ResponseBody String listByPageForDemander(String currentPage,
			String pageSize, String linkname, String linkphone,
			String serviceOrderId, String type, HttpSession session) {
		JsonUtil jsonUtil = JsonUtil.getInstance();
		...
		demanderSearchModal.setLinkphone(StringUtils.isBlank(linkphone) ? ""
				: linkphone.trim());
		demanderSearchModal.setServiceOrderId(StringUtils
				.isBlank(serviceOrderId) ? "" : serviceOrderId.trim());
	...
	public @ResponseBody String listInsideProviderByPage(String currentPage,
			String pageSize, String linkname, String linkphone,
			HttpSession session) {
		JsonUtil jsonUtil = JsonUtil.getInstance();
		DemanderModel demanderSearchModal = new DemanderModel();
	...
	public @ResponseBody String insideProviderList(String sId,
			HttpSession session) {
		JsonUtil jsonUtil = JsonUtil.getInstance();
	...
	public @ResponseBody String dealDemander(String type, String demanderIds,
		...
		if (StringUtils.isNotBlank(demanderIds)) {
			...
			if (deIds.length > 0) {
				...
				for (int i = 0; i < deIds.length; i++) {
					String id = deIds[i];
					serviceOrder = serviceOrderService.load(Integer
							.parseInt(id));
					if (type.equals("1")) {
						serviceOrder
								.setRemark1(StringUtils.isEmpty(remark) ? ""
										: remark.trim());
						...
						serviceOrder
								.setRemark2(StringUtils.isEmpty(remark) ? ""
										: remark.trim());
